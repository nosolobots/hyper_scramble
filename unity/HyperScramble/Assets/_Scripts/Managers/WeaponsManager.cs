using UnityEngine;
public class WeaponsManager : PersistentSingleton<WeaponsManager>
{
    [Header("Bomb Settings")]
    [SerializeField] GameObject bombPrefab;
    [SerializeField] AudioClip dropBombSound;
    ...
    public void LaunchBomb(Vector3 position)
    {
        if (_bombsLeft > 0)
        {
            ...
            AudioSystem.Instance.PlayFXSound(dropBombSound, position);
    ...
    public void FireCannon(Vector3 position)
    {
        ...
        AudioSystem.Instance.PlayFXSound(fireLaserSound, position);
